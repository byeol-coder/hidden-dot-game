...
**다이얼로그(튜토리얼/결과/도움말/촉각판) 안에서는**: Pan Left·F1 = 이전 버튼, Pan Right·F2 = 다음 버튼으로 포커스 이동, F3 = 선택, F4 = 현재 포커스 읽기, Pan All = 다이얼로그 내용 전체 읽기, LPF1 = 닫기 버튼이 있으면 닫기. 키보드는 Tab/Enter/Esc로 동일하게 동작합니다.

**처음 진입하는 홈(타이틀) 화면에서도**: 키보드 없이 닷패드만으로 시작할 수 있습니다. Pan Left·F1 / Pan Right·F2 = 사운드·연결·나가기·바로 시작·처음부터 배우기 버튼 사이 포커스 이동, F3 = 선택(실행), F4 = 현재 포커스 읽기, Pan All = 화면 소개 듣기.
